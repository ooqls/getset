...
const (
	...
	valkey_passwordFileOpt string = "opt-valkey-password-file"
...
func WithValkeyPasswordFile(path string) valkeyOpt {
	return valkeyOpt{featureOpt: featureOpt{key: valkey_passwordFileOpt, value: path}}
}
...
func (f *ValkeyFeature) apply(opt valkeyOpt) {
	...
	case valkey_passwordFileOpt:
		f.valkeyDB.Auth.PasswordFile = opt.value.(string)
	case valkey_port:
